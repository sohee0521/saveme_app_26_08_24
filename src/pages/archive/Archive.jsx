...
import vector1 from "../../img/vector1.svg"; // 계획대로
import vector3 from "../../img/vector3.svg"; // 빠듯하게
import vector4 from "../../img/vector4.svg"; // 극적으로
...
// 🌟 자연스럽게 살짝 더 물리도록 중심점 좌표 조정
const BUBBLE_CONFIGS = [
  {
    center: { x: 105, y: 74 }, // 상단 원 살짝 아래로 (y: 70 -> 74)
    bgColor: "bg-primary",
  ...
  {
    center: { x: 80, y: 134 }, // 좌하단 원 중심 쪽으로 살짝 이동 (x: 76 -> 80)
  ...
  {
    center: { x: 130, y: 134 }, // 우하단 원 중심 쪽으로 살짝 이동 (x: 134 -> 130)
    bgColor: "bg-[#EAEAEA]",
...
export default function Archive() {
  ...
  const handlePercentageChange = (changedKey, newValue) => {
    const clampedValue = Math.max(0, Math.min(100, Number(newValue)));
    const otherKeys = Object.keys(testPercentages).filter(
      (k) => k !== changedKey,
    );
    const remainingTotal = 100 - clampedValue;

    const currentOtherSum =
      testPercentages[otherKeys[0]] + testPercentages[otherKeys[1]];
    ...
    if (currentOtherSum > 0) {
      newOther1 = Math.round(
        (testPercentages[otherKeys[0]] / currentOtherSum) * remainingTotal,
      );
      newOther2 = remainingTotal - newOther1;
    } else {
      newOther1 = Math.floor(remainingTotal / 2);
      newOther2 = remainingTotal - newOther1;
    }
  ...
  const { topVectorImg, topComment, topStyleName, bubbleItems, totalCount } =
    useMemo(() => {
      ...
      if (isTestMode) {
        ...
        percentages = {
          계획대로:
            total > 0 ? Math.round((counts["계획대로"] / total) * 100) : 30,
          빠듯하게:
            total > 0 ? Math.round((counts["빠듯하게"] / total) * 100) : 60,
          극적으로:
            total > 0 ? Math.round((counts["극적으로"] / total) * 100) : 10,
        };
      }
      ...
      const bubbleItems = sortedStyleNames.map((name, index) => {
        ...
        const size = getBubbleSize(percent);
      ...
      const topVectorImg = STYLE_IMAGE_MAP[topStyleName] || vector3;
      const topComment =
        STYLE_COMMENT_MAP[topStyleName] || "스릴 만점 벼락치기러";
    ...
    }, [doneTasks, isTestMode, testPercentages]);
  ...
  return (
    <div className="w-full mx-auto h-screen bg-background flex flex-col px-[20px] pt-[30px] gap-[25px] overflow-hidden">
      {/* 🧪 [TEST CONTROLLER] */}
      {isTestMode && (
        <div className="p-[10px] bg-yellow-50 rounded-[10px] border border-yellow-300 text-[11px] shrink-0 z-50 flex flex-col gap-[6px]">
          <div className="flex justify-between items-center font-bold text-yellow-900">
            <span>🛠️ 스타일 비율 실시간 테스트 (합계 100% 고정)</span>
            <button
              onClick={() => setIsTestMode(false)}
              className="text-[10px] bg-white px-2 py-0.5 rounded shadow-sm hover:bg-gray-100 cursor-pointer"
            >
              테스트 끄기
            </button>
          </div>
          ...
          <div className="flex gap-1">
            <button
              onClick={() =>
                setTestPercentages({ 계획대로: 70, 빠듯하게: 20, 극적으로: 10 })
              }
              className="px-2 py-1 bg-white rounded text-[10px] border cursor-pointer"
            >
              계획 1등 (70/20/10)
            </button>
            <button
              onClick={() =>
                setTestPercentages({ 계획대로: 20, 빠듯하게: 70, 극적으로: 10 })
              }
              className="px-2 py-1 bg-white rounded text-[10px] border cursor-pointer"
            >
              빠듯 1등 (20/70/10)
            ...
            <button
              onClick={() =>
                setTestPercentages({ 계획대로: 0, 빠듯하게: 0, 극적으로: 100 })
              }
              className="px-2 py-1 bg-white rounded text-[10px] border cursor-pointer"
            >
              극적 100%
          ...
          <div className="flex flex-col gap-1">
            {Object.keys(testPercentages).map((key) => (
              <div key={key} className="flex items-center gap-2">
                <span className="w-[45px]">{key}</span>
                <input
                  type="range"
                  min="0"
                  max="100"
                  value={testPercentages[key]}
                  onChange={(e) => handlePercentageChange(key, e.target.value)}
                  className="flex-1 h-1 bg-yellow-300 rounded cursor-pointer"
                />
                <span className="w-[30px] font-bold">
                  {testPercentages[key]}%
                </span>
              </div>
            ))}
      ...
      <section className="flex flex-col gap-[15px] shrink-0">
        ...
        <div className="w-full bg-white rounded-[10px] px-[25px] py-[10px] flex items-center justify-between min-h-[200px] relative overflow-hidden">
          {/* 좌측: 뱃지 + 캐릭터 + 요약 멘트 */}
          <div className="flex flex-col items-center justify-center gap-[12px] shrink-0 z-20 w-[100px]">
            <h5 className="px-[12px] py-[4px] bg-secondary/40 text-primary !font-bold rounded-full">
              1위 {topStyleName}
            </h5>
      ...
      <section className="flex-1 flex flex-col min-h-0 pb-[55px] gap-[15px]">
        ...
        {filteredTasks.length === 0 ? (
          <div className="w-full bg-white rounded-[10px] p-[30px] flex flex-col items-center justify-center text-center">
            <h5 className="text-dark-gray">
              {selectedFilter === "전체"
                ? "아직 완료된 일정이 없습니다."
